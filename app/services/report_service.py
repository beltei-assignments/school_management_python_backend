...
def get_all_reports(
    ...
    query = (
        ...
        .options(joinedload(ProgressReport.student), joinedload(ProgressReport.subject))
    ...
    return {"count": total_count, "rows": report_list}


def create_report(db: Session, report_payload: report_schema.ReportCreate):
    report_exist = (
        db.query(ProgressReport)
        .filter(
            ProgressReport.student_id == report_payload.student_id,
            ProgressReport.subject_id == report_payload.subject_id,
            ProgressReport.term == report_payload.term,
        )
        .first()
    )
...
def update_report(
    db: Session, report_id: int, report_payload: report_schema.ReportUpdate
):
    try:
        report = (
            db.query(ProgressReport)
            .filter(ProgressReport.disabled == False, ProgressReport.id == report_id)
            .first()
        )
        if report is None:
